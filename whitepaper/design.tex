...
Our solution proposes a protocol where a user is identifiable across app stores operating within the protocol. This identification does not contain any link to personal information (for example, email, phone number, etc), thus user anonymity is maintained. Since users are identifiable across stores, a user cannot be attributed to the same campaign in different stores, thus solving the risk \textsf{R1.2}. \\
