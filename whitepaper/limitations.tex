...
One important concern is naturally the possibility of fraud by a (fake) user to earn more tokens. The only certainty is that is not possible to avoid totally fraud. The mobile phone is an untrusted environment that can be tampered. Today, fraud in installation attributions exists, as seen in videos of farms of smartphones where programatic clicks are done in baners or installing apps.

AppCoins protocol tries to improve the current state-of-art: 
\begin{itemize}
\item Trade-off between the effort needed to explore the attack vector and the benefits extracted 
\item User Fingerprint:
